...
namespace ImageGallery.Client.Controllers
{
    ...
    public class GalleryController : Controller
    {
        ...
        public async Task<IActionResult> OrderFrame()
        {
            ...
            {
                throw new Exception("Unable to discover endpoint", metadataResponse.Exception);
            }

            var accessToken = await HttpContext.GetTokenAsync(OpenIdConnectParameterNames.AccessToken);

            var userInfoResponse = await idpClient.GetUserInfoAsync(new UserInfoRequest() { Address = metadataResponse.UserInfoEndpoint, Token = accessToken });

            if (userInfoResponse.IsError)
            {
                throw new Exception("Unable to access UserInfoEndpoint", metadataResponse.Exception);
            }

            var address = userInfoResponse.Claims.FirstOrDefault(x => x.Type == "address").Value;
